...
def plotter(train_loader, batch_size, output_path, mode):
    for i_batch, batch in enumerate(train_loader, total=len(train_loader), desc='Plotting Samples from {} set'.format(mode)):
...
data_path = '/ibiscostorage/mdelliveneri/EUCLID/version2/results'
...
target_info = {}
targets_files = [os.path.join(targets_dir, file) for file in os.listdir(targets_dir) if target_name in file]
target_info['targets'] = np.array(targets_files)
target_info['ids'] = np.array([int("".join([t for t in tid.split('_')[0].split('/')[-1] if t.isdigit()])) for tid in targets_files])
target_info['orientations'] = np.array([int(tid.split('_')[1][1]) for tid in targets_files])
target_info['len'] = len(targets_files) // len(np.unique(target_info['orientations']))
...
print('\n')
print('and the following target:')
print(target_name, target_info['len'], len(np.unique(target_info['orientations'])), len(target_info['ids']))
print('Number of simulations in catalogue: ', len(idlist))
catalogue = catalogue[catalogue['subhalo ID'].isin(target_info['ids'])]
idlist = catalogue['subhalo ID'].values
print('Number of simulations in catalogue after filtering based on targets: ', len(idlist))
...
for id in idlist:
    fig, axs = np.subplots(nrows=n_orientations, ncols=len(bands), figsize=(20, 20))
    for o in np.arange(1, n_orientations + 1):
        for b, band in enumerate(bands):
            idx = np.where(band_info[band]['ids'] == id)[0]
            files = band_info[band]['files'][idx]
            orientations = band_info[band]['orientations'][idx]
            oidx = np.where(orientations == o)[0]
            file = files[oidx]
            data = load_fits(file)
            im = axs[o, b].imshow(np.log10(data + np.min(data) + 1e-10), origin='lower', cmap='magma')
            axs[o, b].axis('off')
            axs[o, b].set_title('{} {}'.format(band, orientations[o]))
            fig.colorbar(im, ax=axs[o, b])
    plt.tight_layout()
    plt.title('ID: {}'.format(id))
    plt.savefig(os.path.join(output_path, 'Image_{}.png'.format(id)))
