...
def get_big_cube_dataloaders(root_dir, training_transforms, validation_transforms, patch_size, patch_overlap, batch_size, 
                            queue_length, samples_per_volume,
                            num_workers=4, multi_gpu=False, inference=False):
    if inference is False:
        clean_file = [file for file in os.listdir(root_dir) if 'clean' in file][0]
        dirty_file = [file for file in os.listdir(root_dir) if 'dirty' in file][0]
        clean_path = os.path.join(root_dir, clean_file)
        dirty_path = os.path.join(root_dir, dirty_file)
        sample = tio.Subject(
            dirty=tio.ScalarImage(dirty_path, reader=ALMABigCubeReader),
            clean=tio.ScalarImage(clean_path, reader=ALMABigCubeReader),
        )
        patch_sampler = tio.data.UniformSampler(sample, patch_size, patch_overlap)
        dataset = tio.SubjectsDataset([sample], transform=training_transforms)
        
        if multi_gpu == True:
            subject_sampler = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True, 
                                                     sampler=DistributedSampler(dataset))
        else:
            subject_sampler = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
        ...
        dataloader = torch.utils.data.DataLoader(patch_queue, batch_size=batch_size, num_workers=num_workers, pin_memory=True)
        return dataloader
    
    else:
        file = root_dir
        sample = tio.Subject(
            dirty=tio.ScalarImage(file, reader=ALMABigCubeReader))
        patch_sampler = tio.data.GridSampler(sample, patch_size, patch_overlap)
        if multi_gpu == True:
            dataloader = torch.utils.data.DataLoader(patch_sampler, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True, 
                                                     sampler=DistributedSampler(patch_sampler))
        else: 
...
def get_big_cube_patches(path, patch_size, patch_overlap, batch_size, num_workers=4, multi_gpu=False):
    sample = tio.Subject(
...
def get_mosaic_inference_dataloader(root_dir, model_input_dim, cnn_padding, batch_size, multi_gpu=False):
    fits_file = os.listdir(root_dir)[0]
    desired_dim = model_input_dim - 2 * cnn_padding
    header = fits.getheader(os.path.join(root_dir,fits_file))
    cube_shape = np.array(list(map(lambda x: header[x], ['NAXIS1', 'NAXIS2', 'NAXIS3'])))
    desired_dim = np.minimum(desired_dim, cube_shape)
    print('Found cube of shape {}'.format(cube_shape))
    print('Creating patches of shape {}'.format(desired_dim))
    data_cache = CubeCache(os.path.join(root_dir,fits_file), gradual_loading=True)
    slices_partition = partition_expanding(cube_shape, desired_dim + 2 * cnn_padding, cnn_padding)
    slices_partition = np.array_split(slices_partition[0], 1)[0]

    def MosaicReader(slices):
        data_cache.cache_data(slices)
        hi_cube_tensor = data_cache.get_hi_data(slices)
        ...
        affine = np.eye(4)
        return model_input, affine     

    samples = list()
    for slices in slices_partition:
        sample = tio.Subject(
            hi_cube=tio.ScalarImage(tensor=MosaicReader(slices)[0], affine=MosaicReader(slices)[1],
...
def preprocess(images_path: str, training_path: str, file_name : str, train_num : int):
  """
  ...
  numpy_array = load_fits(images_path)
  # rotate the image
  lst=[]
  ...
  if (numpy_array.shape[0] > 1):
    ...
    del numpy_array
  return(print("the datacubes are preporcessed and saved"))
...
def get_subject_list(training_path: str):
    subject_lst = []
...
def get_dataloader(training_path: str, batch_size: int, patch_size: tuple, queque_lenght: int, 
                    num_workers: 0):
    subject_lst = get_subject_list(training_path)
    sampler = tio.data.UniformSampler(patch_size)
    #transform = tio.Compose([
    #    tio.RandomFlip(),
    #    tio.RandomAffine(),
    patches_queue = tio.Queue(subject_lst, sampler, queque_lenght, num_workers)
    dataloader = DataLoader(patches_queue, batch_size, num_workers)
    return dataloader
...
class Trainer:
    def __init__(self, config: dict) -> None:
        ...
        elif self.config['dataset'] == 'BIG_CUBE':
            self.train_data, self.valid_data = get_big_cube_dataloaders(self.config['data_path'], training_transforms, validation_transforms, 
                                                                       self.config['patch_size'], self.config['batch_size'], self.config['queue_length'], 
                                                                       self.config['samples_per_volume'], self.config['num_workers'], self.config['multi_gpu'], 
...
def is_main_process():
    return get_rank() == 0       
class Tester:
    def __init__(self, config: dict, device_id, ouput_tensor) -> None:
        self.config = config
        self.output_tensor = ouput_tensor
    ...
    def multi_gpu_inference(self):
        ...
        if is_main_process():
            ...
            header = get_header(self.config['inference_path'])
...
def inference_multigpu(config):
    ...
    device_id = ddp_setup()
    output_tensor = [None for _ in range(config['num_gpus'])]
    if is_main_process():
    ...
    if is_main_process():
        ...
        run_id = wandb_run.id
        config['model_name'] = config['dataset'] + '_' + config['name'] + '_' + config['dmode'] + '_' + str(run_id)
